...
関数すらキーにできます。これだけ聞けば「ハッシュの章で見た辞書」と
...
埋め込み文字列とロープ ── 本書が繰り返し見てきた「一つの型に見せて、
...
> オブジェクトの章のシェイプや、JIT の章のフィードバックと同じ
...
ただ一つで、それが素直に長さになります。ところが**穴がある**と、境界は
...
[](#cite:ierusalimschy2005)。値の表現の章で見た「Java の `Integer`
...
ハッシュ部の中身も、ハッシュの章で見たのとは少し違う、Lua 独特の工夫が
...
鎖は配列の外ではなく**配列内の空きスロット**へ `next` でつながるので、
...
オブジェクトの章で見たメソッド探索を思い出してください。Lua では
...
> （LuaJIT など）はこの探索をインラインキャッシュ（オブジェクトの章）で
...
これで個性派言語の旅は本当におしまいです。次章では、本書全体を振り返り、
ここまで見てきた数々のデータ構造に通底する設計の原理を、改めて言葉に
してみましょう。
