...
- 小さな値のためにヘッダのぶんメモリが膨らむ（CPython の整数は
...
専用の小さなクラス表を引きます。「即値」の発想を文字列にまで広げた
...
利用者には一枚の `Float` として見せます。即値とヒープの二段構えという、
本書で繰り返し現れるパターンの好例です。
...
LuaJIT などが採用しています。利点は浮動小数点数の演算が一切の変換なしで
できることです。タグ付きポインタ方式（Flonum）が浮動小数点数のために
...
（monomorphization）、すなわち多相的なコードを、使われる型ごとに**別々の機械語
コードとして複製してしまう**方法です。
...
（sum type、タグ付きユニオン、バリアント、enum とも）、つまり「いくつかの形の
**どれか一つ**を取る値」です。`Result` は成功か失敗のどちらか、構文木の
ノードは整数か二項演算か（[構文木の章](syntax-tree.md)）、というあれです。直和型を表すには、
...
います。問題は、C ではタグと共用体の対応を**誰も強制しない**ことです。
...
- **インライン（非ボックス）**：Rust の `enum` は、判別子と「最大の枝が
  ...
  詰まります。読むときはまず判別子を見て、枝に応じて中身を解釈します。
...
全部のフィールドを**まとめて**持つ）です。[オブジェクトの章](objects.md)のシェイプが
...
列指向データベースや Apache Arrow（[直列化の章](serialize.md)）、ゲームの ECS、NumPy の
構造化配列、APL 系（[APL の章](apl.md)）の列の世界は、いずれも SoA 側に振った
設計です。中身は同じデータなのに、AoS と SoA で性能が桁違いに変わります。
[ハッシュの章](hashes.md)の SwissTable や[平衡木の章](trees.md)の B木と同じく、**メモリ階層が
レイアウトの良し悪しを決める**時代を映しています。[配列の章](arrays.md)で見た
...
太った値、単相化、ニッチ最適化は、どれも同じ問いへの異なる解であり、
