...
ではなく一括管理の構造を —— これもデータ構造選びの定石の一つです。
...
正規表現型の章で見る「バックトラック＋メモ化で線形化」（Ruby 3.2 の
...
解析**（incremental parsing）—— 1 文字打つたびにファイル全体を
解析し直すのではなく、**変更の影響範囲だけ**を再解析することです。
...
まるで違う —— 同じ「構文木」という言葉に騙されてはいけません。

## ポインタをやめて配列にする：フラットなAST
...
- **パターンマッチのコンパイル**：`case` 式の各パターンを**決定木**
  ...
  正規表現の章の DFA 対バックトラックと同じ構図が、パターンマッチの
  コンパイルにも現れます。
...
きました。次の章からは、プログラムを**実行する**ための土台 ——
値の表現、スタックとフレーム、メモリ管理 —— に進みます。
