...
O(1) です。一周を超える未来は、粗いスロットの上位ホイールに置いて
順次繰り下ろす**階層タイミングホイール**で扱います（時・分・秒の
針の関係そのものです）。Linux カーネル、Netty、Rust の Tokio など、
大量のタイムアウトを抱えるシステムの定番です。発火順が厳密に
ほしい（ヒープ）か、ほぼ全部キャンセルされる大量のタイムアウトを
O(1) で出し入れしたい（ホイール）か —— ネットワーク I/O の
タイムアウトは後者の典型なので、ランタイムの用途で選択が
分かれます。
