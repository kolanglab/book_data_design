...
（つまり多少遅れて）発火させてしまうのです。乱暴に見えますが、
ここで効くのが用途の観察です —— カーネルのタイマーの大半は
ネットワーク I/O のタイムアウトで、**ほとんどが発火前にキャンセル
される**ため、遠い未来の発火精度を保証する意味がそもそも薄いの
でした。発火順・発火時刻が厳密にほしい（ヒープ）か、ほぼ全部
キャンセルされる大量のタイムアウトを O(1) で出し入れしたい
（ホイール。ただし精度は妥協）か —— ランタイムの用途で選択が
分かれます。
