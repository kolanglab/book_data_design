...
[ハッシュの章](hashes.md)で見たとおり、「キーで引く」だけならハッシュ表が平均 O(1) で
...
- **近傍検索**：「x 以上で最小のキーは？」（データベースの索引、
  タイマーの「次に発火するのはどれ？」）
- **最悪時間の保証**：衝突攻撃（[ハッシュの章](hashes.md)）の心配がない O(log n)
...
`Data.Map` といった標準ライブラリの「ソート済みマップ」の正体を、
この章で解剖します。
...
木の高さに比例します。問題は、素朴な BST の高さが挿入順に
依存することです。ソート済みのキーを順に挿入すると、木は一度も
枝分かれせず、一本の鎖に退化します。高さ n、すべての操作が O(n)。
連結リストと同じです。
...
O(log n) に保ち続ける仕組み（平衡化）が必要になります。
...
配分が違うわけです。[ハッシュの章](hashes.md)の「負荷率をどこに置くか」と
同種の、細かいが効く設計判断です。
...
平衡木が最も輝くのは、実は関数型言語です。[配列の章](arrays.md)で見た
...
[ハッシュの章](hashes.md)の HAMT が「順序なし永続マップ」の解だとすれば、
...
キャッシュミス**になりがちなのです（[リストの章](lists.md)で見た連結リストの
弱点と同じです）。

Rust が標準の順序付きマップとして赤黒木ではなく **B木**
（`BTreeMap`）を選んだのは、この弱点への直接の回答です。B木は
データベースとファイルシステムの索引のために生まれた多分木で
[](#cite:bayer1972)、1 ノードに多数のキーを配列で詰め込みます
（Rust は 1 ノードあたり最大 11 キー）。木の高さは log₆ n 程度まで
潰れ、ノード内はキャッシュに載った配列の線形・二分探索で済みます。
ポインタ追跡を減らし、連続メモリの走査に置き換える。
[ハッシュの章](hashes.md)の SwissTable とまったく同じ「メモリ階層が古典を
作り直させた」物語です。
...
ただし、階層化した瞬間に O(1) の看板にはただし書きが付きます。
...
作り直しでカスケードを廃止しました [](#cite:corbet2015)。遠い
...
2 回引っ越しました。ほぼ確実に途中でキャンセルされる運命
なのに、です。新設計では第 3 段に置かれたまま一度も動かず、
キャンセルならリストから O(1) で外れて終わり、万一発火するなら
最大 512 ms（粒度ぶん）遅れます。30 秒の約束が 30.5 秒になる程度で、
...
`HashedWheelTimer` は既定で 100 ms × 512 スロットの一段だけ
...
本章前半の主役である赤黒木です。発火順・発火時刻が厳密にほしい
...
次の章では、コレクションを束ねて「もの」を表す仕組み、すなわち
オブジェクトの実装に進みます。
