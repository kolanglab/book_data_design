...
cons セルは、たった**二つの参照を持つ箱**です。慣習的に、先頭の値を
...
Lisp・Haskell・OCaml・Erlang・Scala といった関数型の系譜では、
今日でもこのリストが**基本のコレクション**です。配列ではなくリストが
...
セルを 1 個作り、その cdr で既存の `list` を**共有**しているだけです。
つまり cons セルのリストは、
...
という性質を、**何の工夫もなしに**満たします。配列型の章で見た HAMT が
...
**入れ子のリストを作るだけ**にして、最終的な出力（ソケットへの書き出し
など）の段階で、I/O 層が木をたどりながら断片を順に書き出します。
平坦化を最後まで遅らせる、ロープ（文字列型の章）の思想の Erlang 版です。
...
—— この発想は「遅延評価」の章でさらに深掘りします。

## キューと両端キュー：純粋関数型の知恵

リストは先頭しか速くないので、**キュー**（queue、先入れ先出し）を
作るには工夫が要ります。有名なのが**二本のリストでキューを作る**
...
配列型の章で見たとおり、成長の仕組みが一切不要だからです。

タプルの要素に**名前**を付けたものが**レコード**（record）や構造体です。
ここで思い出してほしいのが、シンボルテーブルの章の「名前は早い段階で
番号に消す」原則です。静的型付け言語の構造体アクセス `p.x` は、
コンパイル時に「先頭から 8 バイト目」という**固定オフセット**に解決
され、実行時には名前が存在しません。動的型付け言語のレコード（Ruby の
`Struct` や Python の `namedtuple`）は、名前→位置の対応表を**クラス側に
一枚だけ**持ち、インスタンスは値の配列だけを持ちます —— これは次章で
扱う「オブジェクトの実装」の縮図そのものです。
...
なる代わりに、**整列順での反復**と**範囲検索**（「10 以上 20 未満の
...
変数の集合」などの解析（構文木の章で触れた CFG 上のデータフロー解析）が
ビット集合の演算として実装されます。
...
なお、シンボルテーブルの章で出てきた**スキップリスト**も「整列を保つ」
...
※ CRuby の `shift` は開始位置をずらして O(1)（配列型の章）。
※2 部分木サイズを持つ順序統計木の場合（平衡木の章）。
