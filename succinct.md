...
ここまでの章で組んできた構造は、どれもポインタを潤沢に使ってきました。
平衡木の章のノードは `left`・`right` の2本のポインタを持ち、64bit 機なら
それだけで 1 ノード 16 バイト＝**128 ビット**を、中身の値とは別に
消費します。連結リスト（リストの章）もハッシュの鎖（ハッシュの章）も
同じで、「要素をつなぐ」ためのアドレスが、データ本体と同じか
それ以上の場所を食うのが常でした。

ふだんはそれで困りません。しかし扱う構造が**巨大で・静的で・主記憶に
載せたい**とき、この「つなぎのコスト」が問題の主役になります。
言語処理系の周辺では、そういう構造が現実に現れます。

- **辞書**：形態素解析器（MeCab・Sudachi）や仮名漢字変換（IME）が
  抱える、数十万〜数百万語の見出し語の集合。
- **キーワード集合**：予約語・組み込み関数名・Unicode の文字属性表のような、
  起動時に読み込む大きな定数表。
- **全文索引**：ソースコードやログを「部分文字列で検索する」ための索引
  （正規表現の章で見た探索の、巨大データ版）。

これらを素朴なトライ（文字列の章）やハッシュで持つと、見出し語の
文字数の何十倍ものメモリを食います。そこで問うのです ——
...
> [!NOTE]
> rank/select は「タグとニッチ」（おわりにの設計パターン）とは逆の
> 発想です。タグは余ったビットに情報を**足し**ますが、簡潔構造は
...
> [!TIP]
> 木を文字列に符号化するこの発想は、直列化の章の「木を一列に
> 並べる」と地続きです。違いは目的で、直列化が**復元**のために
...
正規表現の章では、与えられた文字列の中をパターンが舐めていきました。
では逆に、巨大なテキストを**前もって索引化**し、任意の部分文字列が
...
「圧縮率」と「検索速度」を同時に取る —— ハッシュの章の SwissTable や
平衡木の章の B木と同じ、メモリ階層の時代ならではのデータ構造です。

## 言語処理系の現場で

簡潔データ構造が言語処理の周辺で最もよく顔を出すのは、
**巨大な静的辞書をトライで持つ**場面です。トライ（文字列の章）は
共通接頭辞を畳んで語の集合を表しますが、ノードごとにポインタや
遷移表を持つ素朴な実装はメモリを食います。ここに簡潔表現が効きます。

- **mozc**（Google 日本語入力のオープンソース版）は、変換辞書のトライを
  **LOUDS** で表す `louds_trie` を内部に持ちます。数十万語の辞書を
  小さく抱えたまま、入力に対する接頭辞検索を高速に行うためです。
- **marisa-trie**（Yata による）は、トライを再帰的に LOUDS 化した
  ライブラリで、辞書圧縮の定番として各種の IME・全文検索・
  Python バインディング経由のツールで使われています。
- 一方、形態素解析器 **MeCab** が使う **ダブル配列**（double-array trie）は
  厳密には簡潔構造ではなく「2 本の整数配列で遷移を表す」compact 寄りの
  方式です。遷移が配列1回参照で済み**更新も比較的しやすい**ので、
  「静的で極限まで小さく」を狙う LOUDS トライとは別のトレードオフ点に
  います。同じ「辞書トライ」でも、更新頻度と要求メモリで設計が
  分かれる —— 本書で何度も見た構図です。

辞書以外にも、構文木（構文木の章）を簡潔表現で持って巨大なソースを
省メモリで解析する研究や、JIT（JIT の章）が抱える各種の側用テーブルを
詰める応用などがあります。共通するのは「**読み出しは膨大だが書き換えは
ほとんどない、静的で巨大な構造**」という条件です。
...
- 言語処理の現場では**辞書トライ**（mozc の LOUDS、marisa-trie）が代表例。
  ダブル配列との対比に「更新頻度 × メモリ」のトレードオフが現れる
- 代償は更新の難しさ・定数項・実装難度。**静的で巨大**な構造でこそ報われる

次の章では、ここまで見たコレクションを束ねて「もの」として
振る舞わせる仕組み —— オブジェクトの実装に進みます。
