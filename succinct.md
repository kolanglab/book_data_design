...
0 が n+1 個。合計 2n+1 ビットで木の形がすべて入りました。しかも、
...
書きます。やはり開き括弧と閉じ括弧が n 個ずつで 2n ビット。`(` を 1、
...
（Bowtie や BWA など、数十億塩基を数ギガバイトに収めて検索する道具）の
...
表です。本章の冒頭で触れた CRuby（MRI）の **`succ_index_table`** が、
...
これは、ビットを詰めた即値の小ブロックと、その累積を持つ大ブロックの
二段構えという、本章で組んだランク辞書そのものです。例外バックトレースや行カバレッジ、
...
でした。研究の世界では、構文木（[構文木の章](syntax-tree.md)）そのものを LOUDS/BP で簡潔
表現し、巨大なソースを省メモリで扱う方向も探られています。

> [!NOTE]
> 「トライを LOUDS で畳んで巨大な辞書を小さく持つ」という応用は、
> 形態素解析器（MeCab や Sudachi）や仮名漢字変換（IME。mozc は LOUDS トライ
> を内部に持つ）といった**自然言語処理**の分野でとりわけ有名です。使う
> 技法は本章とまったく同じですが、これらはコンパイラやインタプリタ
...
> 1 要素の挿入や削除に rank/select 索引の作り直しが要ります（動的化の
...
> 「表現を選ぶ」という本書の主題どおり、これも要求に応じた一つの
> 選択肢であって、既定値ではありません。
...
- ポインタ表現は「つなぎ」に大量のビットを使う。巨大で静的な構造では
  ...
  まで縮める動機が生まれる
- 下限＋ o(n) に張り付きつつ操作できるのが簡潔データ構造。土台は
...
- 代償は更新の難しさ、定数項、実装難度。静的で巨大な構造でこそ報われる
