...
- **言語そのものを相手にする辞書**：形態素解析器（MeCab・Sudachi）や
  仮名漢字変換（IME）が抱える、数十万〜数百万語の見出し語の集合。
...
この「つなぎのコスト」が主役に躍り出ます。たとえば識別子表や辞書を
素朴なトライで持つと、見出し語の文字数の何十倍もの
メモリを、ポインタが食い尽くします。そこで問うのです ——
...
辞書のような「大きな専用構造」だけが舞台ではありません。**ありふれた
処理系の足元**にも簡潔表現は潜みます。CRuby（MRI）は、バイトコード列
`iseq` に付随する **`insns_info`** —— 各命令の位置（プログラムカウンタ）
から、その命令のソース行番号やトレースイベント情報を引く表 —— を、
簡潔ビットベクトルで圧縮しています（内部名 `succ_index_table`、Ruby 2.6
で導入）。隣り合う命令はたいてい同じ行に属するので、「情報が変わる命令
位置」はまばらです。そこを 1 のビットで印してビットベクトルに畳み、PC が
与えられたら **rank** でエントリ番号を引く —— ビットを詰めた即値の小
ブロックと、その累積を持つ大ブロックの二段構えという、本章で組んだ
ランク辞書そのものです。例外バックトレースや行カバレッジ、`set_trace_func`
のたびに引かれるこの表を小さく保つための、文字どおり「内部実装に
ちょっと効かせた」簡潔データ構造の好例です。

このほか、構文木（構文木の章）を簡潔表現で持って巨大なソースを省メモリで
解析する研究や、JIT（JIT の章）が抱える各種の側用テーブルを詰める応用も
あります。共通するのは「**読み出しは膨大だが書き換えはほとんどない、
静的で巨大な構造**」という条件です。
...
- 言語処理系の現場では**辞書トライ**（mozc の LOUDS、marisa-trie）が代表例。
  CRuby も iseq の行番号表をランク辞書で圧縮している（`succ_index_table`）。
