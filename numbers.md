...
> 一枚の `Integer` に見せます。これがデータ構造の切り替えを隠す好例です。
...
**ラップアラウンド**（型幅を法とする剰余に巻き戻り、32 ビットの
`int` なら 2^32、64 ビットの `long` なら 2^64 を法とする）、Rust はデバッグ
...
## 正確さという属性（exact と inexact）

ここまで「誤差のない整数」と「誤差のある浮動小数点数」を別々に見てきました
が、Scheme や Common Lisp はこの違いを値に付いた属性として
扱います。**exact（正確）／ inexact（不正確）** の区別です。同じ「2」でも
`2` は exact、`2.0` は inexact。`(/ 1 3)` は exact な有理数 `1/3` になり、
`(/ 1.0 3)` は inexact な `0.3333…` になります。肝心なのは、exact/inexact が
数学的な値そのものではなく、その値を**どう表現しているか**の区別だという
点です。本章の主題そのものが、言語の表面に属性として顔を出しています。

Scheme（R7RS）はこの属性に**伝播（contagion）の規則**を与えます。exact
同士の演算はできるかぎり exact のまま（整数の和は整数、`1/3 + 1/6` は
`1/2`）。しかし inexact がひとつでも混じれば、一度失った正確さは後戻り
しないため、結果は inexact へ「感染」します。両者は明示変換
`exact->inexact`／`inexact->exact` や、リーダ構文 `#e`（exact 指定）と
`#i`（inexact 指定）で行き来できます。

実装から見ると、exactness は新しいデータ構造ではなく、**ここまで見てきた
表現のどれを使っているか**に対応します。exact 整数は即値／多倍長整数、
exact な分数は有理数（分子と分母の整数の組、次節）、inexact は浮動小数点数。
つまり「fixnum、bignum、ratnum なら exact、flonum なら inexact」と、表現の
種類がそのまま属性を決めます（処理系によっては値にフラグを別途持たせる
設計もあります）。Ruby も `Integer` と `Rational` は誤差なし、`Float` は誤差
あり、という同じ構図を持ちますが、Scheme のように `exact?` という統一した
述語で言語表面に出してはいません。

> [!NOTE]
> exact/inexact は「整数か小数か」とは**別の軸**です。`2.0` は数学的には
> 整数ですが inexact、`1/2` は小数に見えますが exact。だから Scheme では
> `(integer? 2.0)` が真でも `(exact? 2.0)` は偽になります。型（複素数 ⊃
> 実数 ⊃ 有理数 ⊃ 整数）と正確さ（exact／inexact）の二軸で数をとらえるのが
> **数の塔（numeric tower）** の考え方で、`Integer` から `Complex` までを
> 一貫したインタフェースで扱う Ruby の `Numeric` 階層もこの系譜にあります。
...
- **有理数**（`Rational`）：`1/3` を「分子 1、分母 3」の整数の組として
  ...
  計算のたびに約分（最大公約数の計算）が走るため遅く、分子と分母が
  多倍長整数に育ちやすいのが代償です。
...
整数の組としての有理数と、用途に応じて表現を選ぶ、という本書を貫く
テーマがここにもはっきり現れています。
