...
が、Scheme や Common Lisp はこの違いを**値に付いた属性**として正面から
...
**数学的な値そのものではなく、その値をどう表現しているか**の区別だという
...
`1/2`）。しかし inexact がひとつでも混じれば結果は inexact へ「感染」します
── 一度失った正確さは後戻りしないからです。両者は明示変換
`exact->inexact`／`inexact->exact` や、リーダ構文 `#e`（exact 指定）・
`#i`（inexact 指定）で行き来できます。

実装から見ると、exactness は新しいデータ構造ではなく、**ここまで見てきた
表現のどれを使っているか**に対応します。exact 整数は即値／多倍長整数、
exact な分数は有理数（分子・分母の整数の組、次節）、inexact は浮動小数点数。
つまり「fixnum・bignum・ratnum なら exact、flonum なら inexact」と、表現の
種類がそのまま属性を決めます（処理系によっては値にフラグを別途持たせる
設計もあります）。Ruby も `Integer`・`Rational` は誤差なし、`Float` は誤差
