...
オフセットに並べたメモリの塊にできます。`u.age` は「`u` の先頭から
...
**vtable**）を持ちます。メソッドごとに表の中の位置が固定されている
...
番号」の解決がコンパイル時に完了しています（[シンボルテーブルの章](symbol-table.md)の
...
という同じキー文字列を各自のハッシュ表に重複して抱えることになります。

> [!NOTE]
> 考えてみると、同じクラスから作ったオブジェクトはみな同じ顔ぶれの
> インスタンス変数を持ちます。`User` のインスタンスはどれも `@name` と
> `@age` を持つ。だったら「どの変数がどこにあるか」という**地図**は
> オブジェクトごとに持たず、みんなで一枚を共有すればよいはずです。
> この発想が、次節の核心です。

## 形（シェイプ）を共有する実装

そこで実用の処理系は、オブジェクトの状態を二つに分離します。
...
速さと省メモリを同時に得られるわけです。動的言語のオブジェクトが、
...
各インスタンスは値の配列だけを持ちます。[ハッシュの章](hashes.md)で見たコンパクト
...
オブジェクトの「形」は、変数が増えるたびに変化していきます。処理系は
...
別のシェイプに育ちます。先の TIP の「いつも同じ順序で初期化せよ」が、
この 1 行から導かれることも確かめられます。

## シェイプが持つもの

ここまでのシェイプは「どの名前が何番目か」という**レイアウト**だけを
共有し、値はオブジェクト側の配列に置きました。しかし「シェイプに何を
持たせるか」には設計の幅があり、値そのものをシェイプ側に入れる変種が
あります。実はこの分岐は、シェイプの原典である SELF の**マップ**
[](#cite:chambers1989) からすでにありました。SELF のマップはスロットを
二種類に分けます。**代入可能スロット**（ふつうの可変フィールド。マップは
オフセットだけを持ち、値はオブジェクトごと）と、**定数スロット**
（メソッドや定数。値をマップ側に置き、全クローンで共有する）です。メソッドが
マップに同居するからこそ、同じマップのオブジェクトは「同じメソッド群を
持つ＝同じクラスのようなもの」になり、次節のメソッド探索の起点になります。

V8 の隠しクラスもこの線を引き継ぎ、プロパティを定数として記述子に畳む
最適化を持ちます [](#cite:bynens2017)。あるプロパティ（とくに関数＝メソッド）
が常に同じ値なら、その値をマップ側に定数として保持し、JIT は「この呼び出し先は
不変」と見なしてインライン展開できる。値が書き換えられた瞬間に、ふつうの
フィールドへ遷移します。加えて V8 は、値をオブジェクトに埋め込む
（in-object）か別の配列（プロパティバッキングストア）へ追い出すか、
追加・削除が激しすぎるオブジェクトは共有をあきらめて**辞書モード**
（オブジェクトごとのハッシュ）へ落とすか、といった段階も持ちます。

逆に、Ruby の **Object Shapes** [](#cite:seaton2021) [](#cite:issroff2022) は
レイアウトだけを共有する素直な設計で、値はすべてオブジェクト側です。
値までシェイプに入れると JIT 向けの定数畳み込みが効く代わりに、書き換え時の
無効化（次々節）が要る。フィールド集合だけなら単純で予測しやすい。**同じ
シェイプという発明にも、どこまでを共有の地図に載せるかの幅がある**わけです。
...
（名前 → メソッド実体の）ハッシュ表、すなわち**メソッドテーブル**として持って
います。
...
> [!NOTE]
> Ruby には、メソッドとは別の探索規則を持つ名前がもう一種類
> あります。定数（`Foo::Bar` の類）です。定数の探索は二軸で、
> まず**レキシカルスコープ**（その参照が書かれた場所を囲む
> `module`/`class` の入れ子。CRuby 内部では cref と呼ばれる連結
> リスト）を内から外へたどり、見つからなければ次に継承の鎖を
> たどります。同じ `FOO` という参照でも「どこに書かれたか」で
> 結果が変わるのはこのためです。[シンボルテーブルの章](symbol-table.md)のスコープ
...
失効します。問題はこの番号の粒度です。処理系全体で 1 個の番号にすると、
...
[JIT の章](jit.md)の言葉で言えば究極の投機として、型タプルごとにメソッド本体を
まるごと特殊化コンパイルしてキャッシュします（`f(Int, Float64)`
版、`f(Int, Int)` 版…）。単相化（[値の表現の章](values.md)）を実行時に行って
...
そこで、呼び出し箇所に複数のクラスと結果を覚えておく拡張が考えられ
...
よく来るクラスのメソッドを呼び出し箇所へ直接埋め込む（インライン
展開する）ことで、メソッド呼び出しそのものを消し去れるからです
[](#cite:holzle1991)。「実行を観測するデータ構造」が「最適化の判断材料」に
化けるのです。現代の JIT（V8、JVM、YJIT）の根幹は、SELF の時代のこの発見の
延長線上にあります。
...
オブジェクトには、形や値とは別の、もう一つの基本属性があります。
**同一性**（identity）です。「この `Object.new` と、あの `Object.new` は中身が
同じでも別物」という、そのものらしさです。Ruby の `equal?`、Python の
`is`、Java の参照 `==` がこれを問い、`object_id`／`id()`／`identityHashCode`
が同一性を**値として取り出し**ます。

素朴な実装は「アドレスを ID にする」ことです。オブジェクトはヒープ上の
一意な番地を持つので、番地そのものが同一性の代わりになる。CPython の
`id(x)` はまさにオブジェクトのアドレスで、CPython がオブジェクトを
動かさないから成り立ちます。

ところが、この素朴な手は動く GC（コンパクション）と両立しません。
[メモリ管理とGCの章](memory.md)で見たとおり、コピー GC やコンパクションはオブジェクトを
別の番地へ引っ越させます。`object_id` がアドレスだと、GC のたびに同じ
オブジェクトの ID が変わってしまい、同一性として使えません。だから
**位置から ID を切り離し、安定な ID を割り当てて覚えておく**必要が生じます。

```ruby
o = Object.new
a = o.object_id
GC.start
p o.object_id == a            # => true  GC が走っても同じオブジェクトの ID は不変
p Object.new.object_id != a   # => true  別オブジェクトは別 ID
p 42.object_id                # => 85    即値は値から決まる（2*42+1）
```

- **CRuby**：即値（Fixnum・Symbol・`nil` など）の `object_id` は値から計算
  します（Fixnum なら `2n+1`、[値の表現の章](values.md)）。ヒープオブジェクトは、
  `GC.compact`（オブジェクトを動かす）の導入にともない、**初めて
  `object_id` を求められた時に安定な ID を払い出して保存**し、以後は位置が
  変わっても同じ ID を返すよう作り直されました。逆引き
  `ObjectSpace._id2ref`（ID→オブジェクト）は弱参照の表が要り、リークの
  温床になるため非推奨です（弱参照は[メモリ管理とGCの章](memory.md)）。
- **Java**：`identityHashCode` も移動をまたいで不変でなければなりません。
  HotSpot は**初回要求時にハッシュをオブジェクトヘッダ（マークワード、
  [値の表現の章](values.md)）へ書き込み**、以後 GC が動かしてもヘッダごと持ち運ぶことで
  安定させます。ヘッダにわざわざ同一性ハッシュ用のビットが予約されて
  いるのは、このためです。
- **PyPy**：CPython と違い移動 GC なので `id()` はアドレスにできず、CRuby と
  同様に安定 ID を遅延割り当てして表に覚えます。

同一性とは、そのオブジェクトが「どこにあるか」ではなく「どれであるか」です。
位置が動かないうちは位置で代用できますが、動いた瞬間、同一性は**保存すべき
データ**になります。[識別子の章](identifier.md)の「名前に通し番号を振る」と同じ構図が、
ここではオブジェクトそのものに対して現れているのです。

オブジェクトの実装は、これまでの章で学んだ部品の総合芸術でした。
構造体と vtable（静的な極）、ハッシュ（動的な極）、シェイプという形の共有、
配列への値の格納、キャッシュとその無効化、そして位置から切り離された同一性です。
次の章では、オブジェクトと双対の関係にある、もう一つの「振る舞いを
束ねた値」である**クロージャ**を解剖します。
