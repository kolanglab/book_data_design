...
**レイアウトだけ**を共有する素直な設計で、値はすべてオブジェクト側です。
...
[JIT の章](jit.md)の言葉で言えば究極の投機 —— 型タプルごとにメソッド本体を
...
「同一性とは、そのオブジェクトが**どこにあるか**ではなく**どれであるか**」──
...
値の格納、キャッシュとその無効化、そして位置から切り離された同一性 ——
の総合芸術でした。
