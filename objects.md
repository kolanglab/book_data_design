...
メモリ管理とGCの章で見たとおり、コピー GC やコンパクションはオブジェクトを
...
- **CRuby**：即値（Fixnum・Symbol・`nil` など）の `object_id` は値から計算
  します（Fixnum なら `2n+1`、値の表現の章）。ヒープオブジェクトは、
...
データ**になります。識別子の章の「名前に通し番号を振る」と同じ構図が、
