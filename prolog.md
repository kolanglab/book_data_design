...
構文木の章で見たノードと同じものです（識別子の章で見たとおり、
関手名はインターンされたアトムです）。
...
正規表現の章で見たバックトラックです（歴史的には Prolog のほうが
先輩です）。
...
構文木の章の「言語専用の仮想機械を設計する」方法論の、最も美しい
成果の一つです。
...
数個に絞るのです。論理型言語の探索の足元にも、やはりハッシュの章の
表が敷かれています。
...
実行中に書き換わる可変のデータ構造なのです。オブジェクトの章で見た
