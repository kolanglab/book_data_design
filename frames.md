...
なります。ネイティブコードのフレームは、その究極形です。

## CRuby の二層構造をなす仮想機械のスタック
...
引っ越しの手順は具体的にはこうです [](#cite:gocontigstacks)。スタックが
足りなくなると関数の入口（プロローグ）の検査が `morestack` を呼び、
ランタイムの `copystack` が動きます。`copystack` は、(1) 倍の大きさの
新スタックを確保し、(2) 旧スタックの**上端からの差分** `delta`（＝新しい
上端 − 古い上端）を求め、(3) 使用中のバイト列をそっくり `memmove` で
コピーし、(4) 各フレームを巻き戻しながらポインタを付け替えます。鍵は (4)
で、コンパイラの**ポインタマップ**（精密GCが使うのと同じ、語ごとに
「ポインタか否か」を示すビット表）が「ここはポインタ」と印した語だけを
調べ、その値が**旧スタックの範囲内を指していれば** `delta` を足して
付け替えます。ヒープを指す値や、ポインタに見えるだけの整数には触れません。
「旧範囲を指す語だけを、同じ相対位置へずらす」。これが引っ越しの核心です。

フレーム内のローカル変数や引数は、この上のフレーム走査で拾えます。問題は
**フレームの外**からスタックを指すポインタですが、それは少数の決まった
場所（`defer` の登録、`panic` の記録、保存したクロージャ文脈、チャネル
待ちの `sudog`）にしか現れません。ランタイムはこれらも個別に把握して
同じ `delta` で補正します（チャネルの受信スロットは、並行する送信側が
そこへ書き込みうるので CAS で安全に付け替えます）。補正対象がこれだけで
済むのは、**エスケープ解析**が「アドレスがそのゴルーチンのスタックの外へ
漏れる変数はヒープに置く」を保証しているからです。その invariant が
「スタックを指すポインタは、ほぼ自分のフレーム列の中だけ」を成り立たせ、
取りこぼしを原理的になくしています。

> [!NOTE]
> これが**できる**のは Go が精密な処理系だからです。設計文書も「調整して
> よいのは本物のポインタだけであり、ポインタに見えるだけの整数を避けねば
> ならない」と述べています [](#cite:gocontigstacks)。どの語がポインタか
> 確信できない保守的GC（「メモリ管理とGC」の章）がオブジェクトを動かせ
> ないのと、ちょうど表裏です。C のスタックを動かせない理由も同じです。精密な
> ポインタ情報もエスケープ解析もなく、`&ローカル変数` をどこへでも保存
> できてしまうからです。
...
なく、いまのフレームを**置き換えて**しまえばよいのです。実装としては、
