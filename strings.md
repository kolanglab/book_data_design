...
**CRuby**：バイト列を**符号化方式の情報付き**で持ちます。同じバイト列
...
まとめて処理する CPU 命令）**で数十バイトを一括検査**する手法
...
（たとえば大文字化）は、元を書き換えるのではなく**新しい文字列を作って
...
でしたが、このメモリ滞留問題が実害を生み、Java 7 の途中で**コピー方式に
転換**しました（substring は O(n) になったが、滞留は消えた）。
Go は共有方式のまま、必要なら明示的に複製する `strings.Clone` を
提供しています。同じトレードオフに対して、言語によって倒す向きが
逆 —— どちらが正解ということではなく、**何を既定にし、何を利用者の
注意に委ねるか**という設計の問題です。
...
バッファの**カーソル位置に空白地帯（ギャップ）**を置く構造です。
カーソル位置への挿入・削除はギャップの端を動かすだけで O(1)、
...
（n は文字列長、m は相手・結果の長さ。「文字目」はバイト単位の
...
| 不変な連続配列＋共有ビュー（Go、旧 Java） | O(1) | O(n+m) | O(n)（毎回新規） | —（作り直し） | **O(1)** 共有（滞留リスク） |
...
| ピーステーブル（VS Code） | O(log p)※ | — | O(1)（追記バッファ） | O(log p) | O(log p) ビュー |
...
第一に、大きさで表現が**二段**に分かれます。64 バイト以下は
**ヒープバイナリ**としてプロセスのヒープに置かれますが、それを超えると
**refc バイナリ**として**プロセスヒープの外**の共有領域に置かれ、
参照カウントで管理されます。プロセスヒープ側には小さなポインタ
（ProcBin）だけが残ります。Erlang はプロセス間でデータを**まるごと
コピー**する（共有しない）のが原則ですが、**大きなバイナリだけは
...
「部分文字列と共有」でロープが見せた**スライスの共有**を、Erlang は
言語の既定の挙動にしています。そして同じ**落とし穴**も抱えます ──
1 バイトのサブバイナリが、元の巨大な refc バイナリ全体を生かし続けて
しまう。しかも refc バイナリはヒープ外にあってプロセス GC の圧から
...
> [!NOTE]
> バイナリへの**追記**もよく最適化されます。`<<Acc/binary, X>>` を
...
> 共有・ビット粒度・追記最適化 ── 文字列まわりの工夫が、Erlang では
> 一つのバイナリ型に凝縮されています。
...
二分し、前半・後半の照合を行き来することで、**追加メモリ O(1) で
...
スキップ（ビットマスク）**という、正規表現の章で見た「保証と速さの
