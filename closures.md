...
好例です。ジェネレータを提案した PEP 255 は、`yield` の動作を
「関数の実行状態が**凍結される**（frozen）」と表現しました。本書では
これを、あとで温め直す（再開する）ところまで含めて「冷凍」と呼ぶ
ことにします（「凍結」は不変化＝`freeze` の意味で使ってきたので、
区別のためでもあります）。
...
という印さえ覚えておけば、フレームを丸ごと冷凍しなくても続きを
...
enum FetchSum {
    Start,                                // 開始前：何も持たない
    WaitA { fut_a: FetchFuture },         // 中断点 1 で冷凍中
    WaitB { a: u32, fut_b: FetchFuture }, // 中断点 2 で冷凍中（a を保持）
    Done,
}
```
...
| async の Future | 冷凍フレーム（Python/JS）か、状態機械の構造体（C#/Rust、コンパイラ生成） |
