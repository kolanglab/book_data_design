...
引数で渡し、戻り値で返せます。これが**第一級の関数**（first-class function）です。
...
しかも `c1` と `c2` は別々の `count` を持っています。つまりこのラムダは、
...
しかし遅いのです。変数を一つ読むのに鎖をたどってハッシュを引くのでは、
ネイティブコードの「レジスタ＋オフセット」に遠く及びません。

## スタックに置き、逃げる変数だけヒープへ

実用処理系の発想は逆です。「スタックとフレーム」の章で見たとおり、
ローカル変数はフレーム上のスロットに置くのが最速です。クロージャの
...
- **開いた状態**（open）：変数がまだスタック上で生きている間、
  upvalue はスタックのその位置を指すポインタです。読み書きは
  スタック直撃で速い。
- **閉じた状態**（closed）：変数のスコープが終わる瞬間、変数の値を
  upvalue 自身の中へコピーし、以後は upvalue が値の置き場所になる。
...
変数の意味論を「反復ごとに新しい変数」へ言語仕様ごと変更しました。
...
生成時に、実際に使う変数だけを選んでクロージャ自身のレコードに
...
method）、つまり `self` と関数本体の 2 ポインタからなる小さなオブジェクト
...
`resume` メソッドに書き換えられ、ローカル変数は選別の上で
...
構造体に割り付ける」、これは使われる自由変数を解析して必要なものだけを
...
いる相手（内側の Future）は、ヴァリアントの中に値として埋め込まれ、
