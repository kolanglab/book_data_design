...
- シンボルテーブルの章のハッシュ表は、バケットを並べた**配列**でした。
- 識別子の章の ID 管理は、「ID 番号 → 名前」を**配列**で引いていました。
- 構文木の章のバイトコード（そしてフラットな AST）は、**配列**でした。
- 数値型の章の多倍長整数は、桁（リム）を並べた**配列**でした。
- メモリ管理の章のマークビットマップも、ビットの**配列**でした。
